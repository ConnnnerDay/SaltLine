...
| 30 | Onboarding shell | Mobile recording from registration to dashboard | **Complete** — see `apps/web/README.md`'s Status section and the Live checkpoint's session note below. A new `app/onboarding/page.tsx` (static four-step carousel, adapted from `v2/frontend/src/pages/Onboarding.tsx`'s concept per `docs/R1_RECONCILIATION_AUDIT.md`, but with copy grounded in real v1 scope, not v2's deferred species/rig content) that registration now routes through before landing on `/locations`. Verified with a real Playwright-driven 390×844 mobile browser recording (light and dark), registering a real account against real local Postgres/API/web servers and confirming the full redirect chain lands on the real session-gated `/locations` page; `axe-core` 0 violations both color schemes |
...
| 38 | PWA baseline | Installable shell with full offline navigation and graceful degradation, not just last-cached-forecast viewing; authenticated forecasts not cached forever | **Complete** — see `apps/web/README.md`'s Status section (top entry) and the Live checkpoint's session note below. From-scratch build, not an adaptation of `/v2`'s `vite-plugin-pwa` config per `docs/R1_RECONCILIATION_AUDIT.md`'s §3.5 disposition ("adapt the requirement, replace the implementation"). `experimental.useOffline` (soft in-app navigation retry) plus a hand-written `public/sw.js` (runtime-cached app shell for full/hard navigations, an allow-listed set of paths only, a 4-hour TTL on `/forecast/*`/`/share/*` matching the product's existing freshness window, a real `/offline` fallback page). Verified against a real stopped server (not emulated offline), including the TTL-expiry path itself, not just the happy path; `axe-core` 0 violations on `/offline` |
...
| 47 | Degraded-mode UX | Database/API/email/upstream chaos yields actionable UI | **Complete** — see `apps/api/README.md` and `apps/web/README.md`'s Status sections (top entries) and the Live checkpoint's session note below. Found and fixed real gaps by actually breaking each dependency (a stopped local Postgres, a stopped apps/api), not by inspection alone: a global apps/api exception handler replacing Starlette's raw default 500 with a small, never-leaky JSON body (plus a structured trace line for that same request, closing a real sprint-41 observability gap found along the way); `ForecastErrorCard` no longer leaks a raw error message and dev-only troubleshooting text (internal service/env-var names) to production visitors, and gained a real retry action; `PreferencesForm` no longer conflates a load failure with a save failure or silently offers an editable empty form that could overwrite real data; email-send failures during registration/password-reset no longer fail the whole request. Upstream (NWS/NOAA/CO-OPS/NDBC) chaos checked and confirmed already solid (sprints 21-26), using this sandbox's own always-blocked network as free live evidence |
...
**Complete**. Full account in `apps/api/README.md` and `apps/web/
README.md`'s Status sections (top entries) and the sprint ledger row
above. Short version: rather than guess at what "chaos" might expose,
actually broke each real dependency this session had the means to
break (stopped the local Postgres, stopped `apps/api` itself) and read
what the app actually showed. Found real, concrete gaps, not
hypothetical ones: apps/api had no handler for an unhandled exception,
so a genuine database outage returned Starlette's bare
`text/plain "Internal Server Error"` with no structure, and (checked
against the actually-installed Starlette version rather than assumed)
registering `app.exception_handler(Exception)` installs it as
`ServerErrorMiddleware`'s own handler, wrapping `log_requests` from the
*outside* — meaning that request also got no structured trace line at
all, a real sprint-41 observability gap this sprint's own testing
surfaced. Both fixed: a small, never-leaky `{"detail": "internal server
error"}` JSON body (the real traceback stays server-side only), and
`log_requests` gained its own `try`/`except` so that request now gets a
real trace line too. On `apps/web`'s side, stopping `apps/api` and
loading a real forecast page revealed `ForecastErrorCard` was dumping
the raw caught error plus an *unconditional* dev-only troubleshooting
paragraph (naming `apps/api`, `uvicorn`, internal env var names, a link
to this repo's own README) to every visitor regardless of environment —
confusing for a real user, and a genuine internal-implementation
disclosure. Now gated on `NODE_ENV`, with a real "Try again" retry
link in production instead. `PreferencesForm` had a worse bug: a load
failure and a save failure shared one error string shown as if it were
a validation message on an unrelated field, and a load failure still
left the whole form rendering with `EMPTY` defaults a user could
unknowingly submit and overwrite their real saved preferences with —
split into distinct load/save error states, a load failure now replaces
the form with a retryable error card instead. `lib/auth.ts`'s email
hooks had no `try`/`catch` around the actual SMTP send, so a transient
outage would have failed the whole registration or password-reset
request even though the underlying account operation had already
succeeded — wrapped and logged, never rethrown, without changing
Better Auth's already-deliberately-non-committal client messaging.
Upstream (NWS/NOAA/CO-OPS/NDBC) chaos was checked too, not assumed
fine: this sandbox's own always-blocked network to those domains is
free, live evidence, and the existing `SourceStatus`/confidence
machinery (sprints 21-26) already renders it honestly — nothing needed
fixing there. Everything verified live against real running servers in
both failure and recovery states, not just read from the diff: 2 new
apps/api tests (`tests/test_error_handling.py`), 1 new case in
`tests/test_request_logging.py`; full suite 386 passed (was 383).
`ruff`/`ruff format`/`mypy` and `npm run lint`/`build` all clean.
**Next action** for whichever agent picks this up after it merges: the
remaining self-contained, credential-free ledger rows are thin now —
sprints 9/10/28's CAPTCHA/31's map search/45's legal pages remain
flagged pending real credentials or legal review; 42/43 (error
monitoring, privacy-safe analytics) are "Not accepted" but explicitly
don't need a named vendor decision per the product owner's own
guidance ("choose pragmatic, free-tier-friendly tools rather than
spending a sprint deciding") — worth a look next, though it's a real
product-facing tooling choice, not a pure code-quality fix like this
session's last few sprints; everything else is gated on live production
traffic or real hosting.

**Session note (unmerged, this branch, `claude/ecstatic-rubin-mj5c0k`):**
sprint 38 (PWA baseline), previously "Candidate in `/v2`," advances to
**Complete**. Full account in `apps/web/README.md`'s Status section (top
entry) and the sprint ledger row above. Short version: `docs/
R1_RECONCILIATION_AUDIT.md`'s own disposition for this row was "adapt
the requirement, replace the implementation" — v2's `vite-plugin-pwa`
config only cached the last-loaded forecast, and its cache rule's URL
pattern didn't even match this app's real paths, so this is a
from-scratch build, not a port. Two pieces: `experimental.useOffline`
(next.config.ts, Next 16's own connectivity-aware retry for soft
in-app navigations, surfaced via `app/offline-banner.tsx`) and a
hand-written `public/sw.js` for the case that flag explicitly can't
cover — a hard reload or a cold PWA launch — since the framework docs
themselves say "full offline loads would need a service worker."
`sw.js` runtime-caches (not precaches, since Turbopack's build output
is content-hashed per build) an allow-listed set of paths only (`/`,
`/locations`, `/forecast/*`, `/share/*`), deliberately excluding
`/saved`/`/preferences`/`/account` since those render a signed-in
user's personal data and Cache Storage is shared across every account
that signs into a browser profile, not scoped per session — a real
cross-account leak risk on a shared device that this sprint's own
"authenticated forecasts not cached forever" line doesn't literally
mention but the same underlying concern motivates. `/forecast/*`/
`/share/*` additionally get a stamped timestamp and a 4-hour TTL
(matching the product's existing freshness-window decision) — past
that age a cached copy is treated as a miss, not served, since a stale
verdict is worse than an honest "you're offline." A new `app/
offline/page.tsx` (real chrome, not a raw string) is what gets served
for anything else. Verified against a real stopped `next start`
process, not Chromium's `setOffline` emulation (confirmed unreliable
here — it doesn't consistently reach a service worker's own `fetch()`,
which runs in a separate CDP target from the page): with the server
actually down, a previously-visited forecast page served from cache
and a never-visited one rendered the real `/offline` page, not a
browser error; restarting the server made it render live again. The
4-hour TTL was exercised for real too, not just implemented on faith —
a cached entry's timestamp was rewritten to look 5 hours old via the
same `caches` API the worker itself uses, and with the server down
again that URL correctly fell back to `/offline` instead of the stale
content. `axe-core` on `/offline`, both color schemes: 0 violations.
`npm run lint`/`build` both clean. **Next action** for whichever agent
picks this up after it merges: the next self-contained, credential-free
sprint ledger row is harder to find now — 9/10/28's CAPTCHA/31's map
search/45's legal pages remain flagged pending real credentials or
legal review; 46-63 are mostly gated on live production/hosting or
real user traffic. Worth a fresh read of the full ledger (not just this
note's history) for anything the last few sessions' work has newly
unblocked, or a check-in with the product owner on what to prioritize
next.

**Session note (unmerged, this branch, `claude/ecstatic-rubin-mj5c0k`):**
sprint 30 (onboarding shell), previously "Candidate in `/v2`," advances
to **Complete**. Full account in `apps/web/README.md`'s Status section
(top entry) and the sprint ledger row above. Short version: registration
used to route straight back to `/` (the marketing homepage) with no real
onboarding step at all — `app/register/page.tsx`'s own docstring had
flagged this gap since sprint 28. Closed it with a new `app/onboarding/
page.tsx` (a static four-step mobile-first carousel that `RegisterForm`
now routes through, carrying the real destination forward as its own
`?next=`, default `/locations` rather than the old `/` default — a
first-run user has nothing saved yet). Adapted from `v2/frontend/src/
pages/Onboarding.tsx`'s carousel concept per `docs/
R1_RECONCILIATION_AUDIT.md`'s row for this sprint, but not its copy —
v2's steps promised ranked species and bait/rig recommendations, which
`docs/CANONICAL_ROADMAP.md`'s own "Deferred until production evidence"
list excludes from v1; these three steps instead restate `app/page.tsx`'s
own three real value props so onboarding never promises more than the
product already does today. Deliberately no new persisted state (no
`onboarding_completed` column or migration) — nothing in `docs/
product-definition.md`'s "V1 capabilities" list names onboarding-seen
tracking, and the flow only needs to run once, right after a real
signup. Verified against real running `next start`/`uvicorn`/local-
Postgres servers with a real Playwright-driven mobile browser (390×844,
matching an iPhone-class device) recording video of the full flow —
register a real account → step through all four onboarding steps (and
back) → land on the real session-gated `/locations` page, confirmed by
URL and by the account menu rendering (proving the session survived the
redirect chain). Run twice, light and dark; `axe-core` 0 violations on
the first and last step both times. `npm run lint`/`build` both clean.
**Next action** for whichever agent picks this up after it merges:
sprint 38 (PWA baseline, also "Candidate in `/v2`") is the next self-
contained, credential-free candidate; sprints 9/10/28's CAPTCHA/31's map
search/45's legal pages remain flagged, not attempted, pending real
third-party credentials or legal review this session doesn't have.
