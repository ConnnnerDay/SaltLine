...
This PR also renames the FastAPI app title from the placeholder
"Surf & Pier Forecast API" to **"Saltline API"** (`app/main.py`) —
`apps/web`'s Saltline rebrand landed back in sprint 27; `apps/api`'s
OpenAPI `info.title` (visible at `/docs`, `/openapi.json`, and pinned
in `tests/openapi_snapshot.json`) was the one place in this service
still carrying the old name. Regenerated the OpenAPI snapshot
(`python -m scripts.generate_openapi_snapshot`) and reviewed the diff —
one line, `info.title` only, nothing else changed. `ruff`/`ruff
format`/`mypy`/`pytest` (348 passed) all clean; confirmed live against
a real running `uvicorn` server (`GET /openapi.json` → `Saltline API`).
Also caught two more instances of the old name that reach real
external services on every live request, not just docs/tests: the
shared `BoundedHTTPClient`'s default `USER_AGENT`
(`app/infra/http_client.py`, used by NDBC and the station catalogs)
and NWS's/CO-OPS's own override headers
(`app/providers/nws.py`/`noaa_coops.py`) all sent
`(SurfPierForecast, github.com/...)` or `surf-pier-forecast-api/0.1` —
now `Saltline`/`saltline-api`, keeping the same repo URL as the
contact reference NWS's API usage policy asks for (the GitHub repo
itself is still literally named `surf-pier-forecast`, unrelated to the
product's own display name). Updated `tests/test_http_client.py`'s
matching assertion; full check suite re-run clean after this change
too.

This service's first real Postgres connection: **sprint 36
("Preferences")**, `GET`/`PATCH /v1/me/preferences` — the last of the
canonical roadmap's six required `/v1` endpoints, deferred since sprint
25 for needing Better Auth (sprint 28, now shipped in `apps/web`) and a
Postgres-backed store (neither existed then). `app/infra/database.py`
sets up an async SQLAlchemy engine scoped entirely to the `forecast`
schema (`docs/architecture.md`'s ADR-006: separate `auth`/`forecast`
schemas, separate least-privilege roles) via `search_path`, not a
`schema=` argument repeated on every model — a `saltline_api` Postgres
role owns that schema alone, with no grant on `public` or `apps/web`'s
own `auth` schema. `app/domain/preferences.py` is the one model
(`UserPreferences`, keyed by Better Auth's opaque user id — this table
never stores an email, password, or session token, matching ADR-006's
data-ownership split) plus its Pydantic request/response schemas, field
set deliberately narrower than `v2/backend/app/models/profile.py`'s
(gear limitations, accessibility needs, target species, experience
level, theme): `docs/product-definition.md`'s "V1 capabilities" names
exactly "units, fishing style, wind threshold, surf threshold, default
location" — broader personalization would be adding v1 scope indirectly
inside an implementation sprint, which that document's own "Explicit
non-goals" section reserves for a real product decision. Migrations via
Alembic (`migrations/`), the version table itself living in `forecast`
too. `app/api/v1/preferences.py` is the one router that actually
requires a real signed-in user — every other route's
`require_internal_signature` dependency just proves the caller is
`apps/web`'s BFF; this one additionally 401s on an empty `user_id`, the
field ADR-004 always signed but no route ever needed until now.
`PATCH`'s `default_location_id` is validated through the same
`resolve_location_id` every forecast lookup uses, so an unresolvable id
is a real 422, never stored as a working default that later 404s on
every load. 11 new tests (`tests/test_preferences_router.py`) run
against an ephemeral SQLite in-memory database rather than a live
Postgres dependency in CI (`app.infra.database.Base`'s models declare no
`schema=`, so the same ORM code runs against either dialect) — covering
defaults-with-no-row, create-via-PATCH, true partial updates (an
omitted field stays as a prior PATCH left it, an explicit `null` clears
it), per-user isolation, the location-validation 422, and both the 401
and 500 (database not configured) failure paths. Verified against a
real local Postgres too, not just SQLite: a real signed `PATCH` request
against a real running `uvicorn` server, confirmed the row actually
landed in `forecast.user_preferences`. `ruff`/`ruff format`/`mypy`/
`pytest` (359 passed) all clean; OpenAPI snapshot regenerated and
reviewed (new schemas/paths only).

**Sprint 37 ("Saved locations")**: `GET`/`POST /v1/me/locations` and
`DELETE /v1/me/locations/{id}`, the ownership half of the roadmap's
"users can save locations (free tier: 1)" row — the free-tier cap
(`MAX_FREE_TIER_LOCATIONS = 1` in `app/domain/saved_locations.py`) is
recorded as a named constant per the round-2 product decision but not
enforced by any route yet, a follow-up rather than this sprint's job.
Second table in the `forecast` schema, alongside `user_preferences` —
`SavedLocation` deliberately carries no `ForeignKey` to
`user_preferences.user_id`: a preferences row isn't guaranteed to exist
before a user saves a spot, so requiring one first would be an
accidental ordering dependency between two otherwise-independent
features. `POST` validates the `location_id` through the same
`resolve_location_id` preferences' `PATCH` uses (422 on an unresolvable
id) and returns 409 on a duplicate `(user_id, location_id)` save via a
Postgres `UniqueConstraint`; `DELETE` 404s both for a missing row and
one owned by another user, so a request can't distinguish "doesn't
exist" from "exists but isn't yours" and fingerprint another user's
saved-location ids. `require_authenticated_user_id` — the same 401-on-
no-session check preferences' route always did inline — moved to
`app/api/deps.py` so both routers share it rather than each defining
their own. 12 new tests (`tests/test_saved_locations_router.py`): empty
list, save, list-shows-saved, ordering by save time, duplicate 409,
unknown-location 404, delete, delete-unknown-404, cross-user isolation
(404 + no leak of another user's row), and 401 for all three methods
with no session. Verified against a real local Postgres too: real
signed requests against a real running `uvicorn` server, confirmed rows
landing in and clearing from `forecast.saved_location`.
`ruff`/`ruff format`/`mypy`/`pytest` (371 passed) all clean; OpenAPI
snapshot regenerated and reviewed (new schemas/paths only).

**Sprint 49, closing out ("SEO and sharing")**: `GET /v1/locations` — the
one piece that row's own last-open note named, an unfiltered
enumeration of every curated location that `/v1/locations/search`'s
query-based shape can't give a caller building a real sitemap.
`app/api/v1/locations.py` gains one thin route
(`list(load_curated_locations())`, the exact same cached loader
`search` already calls, just without its query filter) — no new
provider, no new domain model. `apps/web/app/sitemap.ts` is the actual
consumer: see that repo's README for the full account of what it builds
from this. 2 new tests (`tests/test_locations_router.py`): returns every
curated location, and is unfiltered with no `q` param at all (unlike
`/search`, which 422s without one). `ruff`/`ruff format`/`mypy`/`pytest`
(373 passed) all clean; OpenAPI snapshot regenerated and reviewed (one
new path only).

**Sprint 45 ("Privacy and deletion")**: `GET /v1/me/export` and
`DELETE /v1/me` — the one piece of that row's acceptance bar buildable
without a real product/legal decision (actual "legal pages" copy needs
real legal review this session can't supply, flagged the same way
sprint 28's CAPTCHA credentials gap is flagged). `app/api/v1/account.py`
is a new router: export returns the literal stored rows for the caller
(preferences fields, saved locations' `location_id`/`saved_at`), not
re-derived data — a saved location's name/state is public curated data
this service could tell anyone, not personal data this account holds,
so it stays out of the export. Delete removes every row this service
holds for the user across both tables it owns
(`user_preferences`, `saved_location`); `apps/web`'s Better Auth
`deleteUser` hook calls this before removing the `auth`-schema account
row itself, so no `forecast`-schema data outlives a deleted account —
verified live, not just wired: a real end-to-end registration → set
preferences → save a location → delete-account flow, confirmed via
direct SQL against both schemas that every row was actually gone
afterward, not just that the login subsequently failed. 7 new tests
(`tests/test_account_router.py`): empty export, export with real data,
delete clears both tables (and preferences' own GET creates a fresh
default row afterward, proving the old row was really gone rather than
just appearing empty from a stale read), delete with nothing stored
still succeeds, per-user isolation, and 401 on both routes with no
...
### `/v1/me/preferences` locally

Needs a real local PostgreSQL database (see `apps/web/README.md`'s
"Authentication locally" for the matching `saltline` database/`auth`
schema setup — this is that same database, a separate schema and role):

```bash
# create a least-privilege role and its own schema, same database as
# apps/web's Better Auth setup
sudo -u postgres psql <<'EOF'
CREATE ROLE saltline_api WITH LOGIN PASSWORD 'dev-password-please-change';
GRANT CONNECT ON DATABASE saltline TO saltline_api;
GRANT CREATE ON DATABASE saltline TO saltline_api;
\c saltline
...
Then run `uvicorn` with `DATABASE_URL` also set (alongside the signing
keys above). Without it, every route except `/v1/me/preferences` still
works fine — that route alone 500s with "database is not configured"
(`app.api.deps.get_db_session`'s fail-closed check) rather than
crashing the whole app at startup.

If you change `app/domain/preferences.py`'s model, generate a new
migration and review it before committing (Alembic's autogenerate
mis-diffs the schema-scoped `alembic_version` table as a spurious
drop/recreate when `version_table_schema` is set — strip those two
blocks out of the generated file, `migrations/versions/
28efa8760686_create_user_preferences_table.py`'s own history is the
worked example):
