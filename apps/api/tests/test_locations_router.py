...
from app.providers.locations import load_curated_locations
...
def test_list_returns_every_curated_location(client: TestClient) -> None:
    resp = client.get("/v1/locations")
    assert resp.status_code == 200
    body = resp.json()
    assert len(body) == len(load_curated_locations())
    ids = [loc["id"] for loc in body]
    assert "wrightsville-beach-nc" in ids


def test_list_is_unfiltered_unlike_search(client: TestClient) -> None:
    resp = client.get("/v1/locations")
    assert resp.status_code == 200
    # No `q` param at all -- this is the one route on this router that
    # doesn't require one, unlike `/search` (test_search_requires_
    # nonempty_query above).
